...
//   a file-backed pool via temp dir so disk I/O, WAL, and page-cache effects are
...
fn bench_find_by_id(c: &mut Criterion) {
    ...
    let tmp_path = std::env::temp_dir().join(format!("solodawn_bench_{}.db", std::process::id()));
    let file_url = format!("sqlite://{}?mode=rwc", tmp_path.display());
    ...
    // Clean up temp file after bench finishes.
...
fn bench_find_by_project(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            ...
            b.iter(|| {
                let pool = pool.clone();
                let _project = project_uuid(&format!("project-{}", iter_idx % 5));
