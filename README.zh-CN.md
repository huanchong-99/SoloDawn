...
**第 4 步：点右上角的确认。** 确认分两步：先确认技术规范（此刻生成并锁定评分规范），再确认质量门配置。**对话完成后记得点确认，不点不会执行**——这是代码层的硬性拦截：两步确认都完成后，工作流才会物化并自动启动。
...
它目前**不擅长对已交付成果的持续迭代**：任务是一次性的——功能 A 交付后想调优、想更新，只能发布一个新任务，不能回到旧任务里继续做。这类场景下会用得比较难受。**使用前请先分辨你的需求属于哪一类。**
