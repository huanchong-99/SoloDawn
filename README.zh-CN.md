...
> ⚠️ **`protoc` 和 `LLVM/libclang` 是构建必需项，但 `scripts/setup-windows.ps1` 不会安装**，需手动安装：
>
> **Windows：** 下载 [`protoc-31.1-win64.zip`](https://github.com/protocolbuffers/protobuf/releases/tag/v31.1)，解压后把 `bin` 加入 `PATH`；再安装 LLVM 并设置 `LIBCLANG_PATH`：
> ```powershell
...
- **本地构建不需要 cmake / nasm / perl**——`libgit2-sys` 通过 `cc` crate 构建，依赖树使用 `ring`（而非 aws-lc）。
